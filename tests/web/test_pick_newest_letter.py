...
(filter type==1 & `_request_letter_*.txt`, newest by ISO-in-name; empty →
...
def test_FS1_multiple_letters_picks_newest_by_iso_name() -> None:
    children = [
        ...
        _item("_request_letter_2026-05-15T143022Z.txt", file_id=2),
        _item("_request_letter_2026-05-14T090000Z.txt", file_id=3),
    ]
    chosen = pick(children, vgm_id=VGM)
    assert chosen["name"] == "_request_letter_2026-05-15T143022Z.txt"
...
def test_FS2_mixed_children_only_request_letters_considered() -> None:
    children = [
        ...
        _item("_request_letter_2026-05-10T000000Z.txt", file_id=12),
...
def test_FS4_single_letter_returns_it() -> None:
    children = [_item("_request_letter_2026-05-15T080805Z.txt", file_id=7)]
    chosen = pick(children, vgm_id=VGM)
...
def test_FS5_non_txt_with_prefix_is_excluded() -> None:
    ...
    letter. (Inverted from the pre-.txt era: `.md` is now the stray.)"""
    children = [
...
def test_FS5_txt_suffix_still_selected_among_non_txt_prefix_siblings() -> None:
    children = [
        _item("_request_letter_2026-05-15T080805Z.md", file_id=1),  # old stray
        _item("_request_letter_2026-05-15T090000Z.txt", file_id=2),
    ]
    chosen = pick(children, vgm_id=VGM)
    assert chosen["document_file_id"] == 2
